...
@Composable
fun DetailScreen(navHostController: NavHostController, book: String, id: Long? = null) {
    ...
    Scaffold(
                ...
                actions = {
                    IconButton(
                        onClick = {
                            ...
                            if (id == null) {
                                viewModel.insert(topic, journal, book, understand)
                            } else {
                                viewModel.update(id, topic, journal, book, understand)
                            }
        ...
        FormSiswa(
            ...
            journal = journal,
            onjournalChange = { journal = it },
            understand = understand,
            onunderstandChange = { understand = it },
...
fun FormSiswa(
    ...
    understand: String,
    onunderstandChange: (String) -> Unit,
    ...
    Column(
        ...
        OutlinedTextField(
            value = journal,
            onValueChange = { onjournalChange(it)},
        ...
        Column {
            radioOption.forEach {
                KelasOption(label = it, isSelected = understand == it, onClick = {onunderstandChange(it)}, modifier = Modifier)
            }
        }
    }
}
...
@Composable
fun KelasOption(label: String, isSelected: Boolean, onClick: (String) -> Unit, modifier: Modifier) {
    Row(
        modifier = modifier.clickable { onClick(label) },
        verticalAlignment = Alignment.CenterVertically
    ) {
        RadioButton(selected = isSelected, onClick = { onClick(label) })
        Text(
            text = label,
...
fun DetailScreenPreview() {
    Mobpro1composeTheme {
        DetailScreen(rememberNavController(), "The rust programming langguage")
    }
}
